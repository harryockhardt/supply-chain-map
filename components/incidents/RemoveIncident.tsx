...
import {removeIncident} from "@/lib/incidents/manage";
import type {CreateState} from "@/lib/incidents/create";
export function RemoveIncident({id,title}:{id:string;title:string}) {
 const [confirm,setConfirm]=useState(false);
 const [state,action,pending]=useActionState<CreateState,FormData>(async()=>removeIncident(id),{});
 ...
 return <form action={action} className="rounded border border-red-200 bg-red-50 p-4">
  <p>Remove “{title}”? It will disappear from the map and incident lists.</p>
