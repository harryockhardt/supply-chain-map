...
export default async function EditIncidentPage({params}:{params:Promise<{id:string}>}) {
 ...
 if(incident.owner_id!==user.id)return <section className="p-6"><h1 className="text-2xl font-semibold">You can only edit your own incidents</h1><Link className="mt-4 inline-block underline" href={`/incidents/${id}`}>Back to incident</Link></section>;
