...
type Props = { point:[number,number]; choices:IncidentChoices; onCancel:()=>void; onChangePoint:()=>void };

export function CreateIncidentForm({point,choices,onCancel,onChangePoint}:Props) {
 const [fields,setFields] = useState({title:"",category_slug:"",description:"",event_start_at:"",status:"unverified",
 current_state_description:"",last_verified_at:"",resolved_at:"",impact_description:"",severity:"2"});
 const [modes,setModes] = useState<string[]>([]);
 const [effects,setEffects] = useState<string[]>([]);
 const [sources,setSources] = useState<{key:number;source_name:string;url:string;published_at:string}[]>([]);
 ...
 const [state,action,pending] = useActionState<CreateState,FormData>(async () => {
  try {
   ...
   return await saveIncident(input);
  } catch(error) {
 ...
 const field = (key:keyof typeof fields,label:string,type="text",required=true) =>
  <label className="block" key={key}>{label}<input className={inputClass} type={type} required={required} value={fields[key]} onChange={e=>set(key,e.target.value)} /></label>;
 ...
 return <section aria-label="Create incident" className="absolute inset-0 z-20 overflow-y-auto bg-white p-5 shadow-xl md:left-auto md:w-[30rem]">
  ...
  <form action={action} onReset={event=>event.preventDefault()} className="mt-4">
   <fieldset disabled={pending} className="space-y-4">
    ...
    {sources.map((source,i)=><fieldset key={source.key} className="space-y-3 rounded border p-3"><legend>Source {i+1}</legend>
     {(["source_name","url","published_at"] as const).map(key=><label key={key} className="block">{key==="source_name"?"Source name":key==="url"?"Source URL":"Publication time (optional)"}<input className={inputClass} type={key==="url"?"url":key==="published_at"?"datetime-local":"text"} required={key!=="published_at"} value={source[key]} onChange={e=>setSources(s=>s.map(x=>x.key===source.key?{...x,[key]:e.target.value}:x))}/></label>)}
    ...
    <button className="block w-full rounded bg-slate-900 px-4 py-3 text-white disabled:opacity-50" disabled={pending}>{pending?"Saving…":"Save incident"}</button>
   </fieldset>
  </form>
 </section>;
}
