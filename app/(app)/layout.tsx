...
export default async function AppLayout({ children }: { children: ReactNode }) {
  const { user, isAdmin } = await requireActor();
  return (
    <div className="flex h-dvh min-h-[32rem] flex-col bg-slate-50 text-slate-900">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <nav aria-label="Main navigation" className="flex gap-4 text-sm"><Link className="underline" href="/map">Map</Link><Link className="underline" href="/my-incidents">My Incidents</Link>{isAdmin && <Link className="underline" href="/admin">Admin</Link>}</nav>
