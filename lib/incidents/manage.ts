...
import { canManageIncident } from "./permissions";
import type { CreateState } from "./create";

function refreshIncident(id:string) {
 revalidatePath("/map"); revalidatePath("/my-incidents"); revalidatePath("/admin");
...
export async function updateIncident(id:string, version:string, value:unknown):Promise<CreateState> {
 ...
 if (!incident || !canManageIncident(actor,incident.owner_id)) return {error:"This incident is unavailable or you do not have permission to edit it."};
 let payload;
...
export async function removeIncident(id:string):Promise<CreateState> {
 const actor=await requireActor();
 const {supabase}=actor;
 const incident=await getIncident(id);
 if(!incident || !canManageIncident(actor,incident.owner_id)) return {error:"This incident is unavailable or you do not have permission to remove it."};
 const {error}=await supabase.rpc("remove_incident",{target_id:id});
 if(error) return {error:error.code==="42501"?"This incident is unavailable or you do not have permission to remove it.":"We could not confirm removal. Check the incident list before retrying."};
 refreshIncident(id); redirect(actor.isAdmin?"/admin?removed=1":"/my-incidents?removed=1");
}
